...
public interface GastoCartaoRepository extends JpaRepository<GastoCartao, Long> {
	...
	@Query(value = "select new io.github.app.dto.query.BancoTotalDto( g.banco, sum(g.valor)) from GastoCartao g "
			+ "where (:banco is null  or g.banco=:banco)" + "group by g.banco")
	List<BancoTotalDto> queryBancoTotal(@Param("banco") Banco banco);

	@Query("SELECT g FROM GastoCartao g " + "WHERE (:banco IS NULL OR g.banco = :banco) "
	...
	List<GastoCartao> queryAll(@Param("banco") Banco banco, @Param("dataInicio") LocalDate dataInicio,
			@Param("dataFinal") LocalDate dataFinal, @Param("nomeRecebedor") String nomeRecebedor,
